...
const OwnerDashboard: React.FC<OwnerDashboardProps> = ({ reports, onResolveReport, students, onUpdateRole, t }) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      {activeTab === 'users' && (
        <div className="glass p-10 rounded-[3rem] border-white/5 shadow-2xl overflow-hidden">
            ...
            <tbody className="divide-y divide-white/5">
              {students.map(u => (
                <tr key={u.id} className="group">
                  ...
                  <td className="py-6">
                    <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${u.role === 'Student' ? 'bg-white/5 text-slate-500' : u.role === 'Faculty' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-cyan-500/10 text-cyan-500'}`}>{u.role}</span>
                  </td>
      ...
      {activeTab === 'reports' && (
        <div className="space-y-6">
          {reports.length > 0 ? reports.map(report => (
            <div key={report.id} className="flex items-center justify-between p-8 bg-red-500/5 rounded-3xl border border-red-500/10 shadow-xl group">
              <div className="flex items-center space-x-6">
                ...
                <div><h4 className="font-bold text-lg">Subject: {report.targetTitle}</h4><p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Reported by {report.reporterName} • Reason: {report.reason}</p></div>
              </div>
              <div className="flex space-x-3">
                <button onClick={() => onResolveReport?.(report.id, 'Dismiss')} className="px-6 py-3 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-transform active:scale-90 shadow-lg">Dismiss</button>
                <button onClick={() => onResolveReport?.(report.id, 'Take Down')} className="px-6 py-3 bg-red-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-transform active:scale-90 shadow-lg">Take Down</button>
              </div>
            </div>
          )) : (
