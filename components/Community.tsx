...
const Community: React.FC<CommunityProps> = ({ currentUser, questions, onAddQuestion, onReport, onViewUser, onFeatureQuestion, onReplyQuestion, challenge, onJoinMeeting, onMeetingPermission, t }) => {
  ...
  const [showReplyModal, setShowReplyModal] = useState<{id: string, title: string, text: string} | null>(null);
  ...
  const handlePostReply = () => {
    if (!replyText.trim() || !showReplyModal) return;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 animate-in fade-in duration-700">
        ...
        <div className="space-y-10 pb-40">
          {filteredQuestions.map(q => (
            <QuestionCard 
              ...
              onReply={() => setShowReplyModal({id: q.id, title: q.title, text: q.text})} 
            />
          ))}
        </div>
      </div>
      ...
      {showReplyModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-8 bg-slate-950/90 backdrop-blur-xl">
           <div className="glass w-full max-w-3xl p-12 rounded-[4rem] border-cyan-500/50 shadow-2xl space-y-8 animate-in zoom-in duration-300">
              <div className="flex justify-between items-start">
                 <div>
                    <h2 className="text-2xl font-bold font-orbitron text-white">Reply to Topic</h2>
                    <p className="text-slate-500 text-sm mt-2">{showReplyModal.title}</p>
                 </div>
                 <button onClick={() => setShowReplyModal(null)} className="w-12 h-12 rounded-2xl bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all">✕</button>
              </div>
              <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5">
                 <p className="text-slate-400 text-sm italic">"{showReplyModal.text}"</p>
              </div>
              <textarea 
                value={replyText}
                ...
                rows={5}
                className="w-full bg-slate-900 border border-white/10 rounded-[2rem] px-8 py-6 text-white outline-none focus:ring-2 ring-cyan-500/30 resize-none"
              />
              <div className="flex gap-4">
                 <button onClick={handlePostReply} className="flex-1 cyber-gradient py-5 rounded-[2.5rem] text-white font-black text-xs uppercase tracking-widest shadow-2xl">{t.submit}</button>
                 <button onClick={() => setShowReplyModal(null)} className="px-12 py-5 rounded-[2.5rem] border border-white/10 font-bold uppercase text-xs tracking-widest">{t.cancel}</button>
              </div>
           </div>
        </div>
      )}
      ...
      {showAskModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-8 bg-slate-950/90 backdrop-blur-xl">
           <div className="glass w-full max-w-2xl p-12 rounded-[3.5rem] border-cyan-500/50 shadow-2xl space-y-8 animate-in zoom-in duration-300">
              <h2 className="text-2xl font-bold font-orbitron">{t.newQuestion}</h2>
              <div className="space-y-4">
                 <input 
                    value={newTitle}
                    onChange={e => setNewTitle(e.target.value)}
                    placeholder="Topic Title"
                    className="w-full bg-slate-900 border border-white/10 rounded-2xl px-6 py-4 text-white"
                 />
                 <textarea 
                    value={newText}
                    ...
                    rows={4}
                    className="w-full bg-slate-900 border border-white/10 rounded-2xl px-6 py-4 text-white resize-none"
                 />
              </div>
              <div className="flex gap-4">
                 <button onClick={handlePostQuestion} className="flex-1 cyber-gradient py-4 rounded-2xl text-white font-bold">{t.submit}</button>
                 <button onClick={() => setShowAskModal(false)} className="px-8 py-4 rounded-2xl border border-white/10 font-bold">{t.cancel}</button>
              </div>
           </div>
        </div>
      )}
...
const QuestionCard: React.FC<{ question: Question, canModerate: boolean, onFeature: (id: string) => void, onReport: any, t: any, currentUser: User, onReply: any }> = ({ question, canModerate, onFeature, onReport, t, currentUser, onReply }) => (
  <div className={`p-10 rounded-[3.5rem] border transition-all shadow-xl group ${question.isFeatured ? 'border-cyan-500/50 bg-cyan-500/10' : 'glass border-white/5 hover:border-white/10'}`}>
    <div className="flex justify-between items-start mb-8">
      <div className="flex items-center space-x-6">
        ...
        <div className="text-left">
          ...
          <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.2em] opacity-60">{question.category} • {new Date(question.timestamp).toLocaleDateString()}</p>
        </div>
      </div>
      <div className="flex items-center space-x-4">
         {canModerate && (
           <button onClick={() => onFeature(question.id)} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${question.isFeatured ? 'bg-cyan-500 text-white border-cyan-500' : 'border-cyan-500/30 text-cyan-500 hover:bg-cyan-500/10'}`}>
              {question.isFeatured ? 'Featured' : t.featureQuestion}
           </button>
         )}
    ...
    <p className="text-slate-400 text-lg leading-relaxed mb-10 pl-2 text-left">{question.text}</p>
    ...
    <div className="flex items-center justify-between pt-10 border-t border-white/5">
       <div className="flex items-center space-x-5">
          ...
          <span className="text-xs font-black text-slate-500 uppercase tracking-widest">{question.answers.length} Responses</span>
       </div>
